...
def get_dialogue_data_for_transformer(max_seq_src, max_seq_trgt,split):
    ...
    dataset = load_dataset("pirocheto/phishing-url",split=split)
    ...
    for data in dataset:
        inputs = list(unidecode(data["url"]))
        outputs = data["status"].split()
...
if __name__ == "__main__":
    x, y, label, src_vocab, trgt_vocab, tokenizer_src, tokenizer_trgt, categories = get_dialogue_data_for_transformer(1700, 2,"train")
