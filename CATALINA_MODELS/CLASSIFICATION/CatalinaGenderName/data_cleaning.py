...
def get_dialogue_data_for_transformer(max_seq_src, max_seq_trgt,split):
    print("\nFetching Dataset Conversational...")
    dataset = load_dataset("erickrribeiro/gender-by-name",split=split)
    ...
    for data in dataset:
        inputs = list(unidecode(data["Name"].lower()))
        outputs = label_map[data["Gender"]].split()

        # add(unidecode(data["Name"]).lower(),"names")
...
if __name__ == "__main__":
    x, y, label, src_vocab, trgt_vocab, tokenizer_src, tokenizer_trgt, categories = get_dialogue_data_for_transformer(27, 2,"test")
    ...
    torch.save(inputs_dict, "test_data.pth")
