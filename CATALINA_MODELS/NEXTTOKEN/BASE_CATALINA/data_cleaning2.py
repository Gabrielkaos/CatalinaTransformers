...
def process_instruction_dataset_tiktoken(
    ...
    for idx, data in enumerate(tqdm(dataset)):
        try:
            ...
            bag_of_words = bag_of_words[:max_seq_len]
            ...
            bag_of_words = bag_of_words + ["<PAD>"] * (max_seq_len - len(bag_of_words))
    ...
    vocabulary = ["<EOS>"]
    vocabulary.extend(get_unique(sequences))
...
if __name__ == "__main__":
    ...
    x,vocab, tokenizer = process_instruction_dataset_tiktoken(
        dataset_name="allenai/dolma3_mix-5.5T-1125",
        max_seq_len=512,
        format_style="simple",  
        split="train"
    )
    
    file_name = "data_base.pth"
    torch.save({
        "x": x,
        "tokenizer": tokenizer,  
        "vocab": vocab,
