...
def prepare_chat_dataset(tokenizer, max_len=512, dataset_name="grimjim/tatsu-lab-alpaca-deduped-instruction"):
   
    
    
    dataset = load_dataset(dataset_name, split="train")
    dataset = dataset.filter(lambda x: any(keyword in x.get('instruction', '').lower() 
                                       for keyword in ['explain', 'reason', 'think', 'analyze', 'why']))
...
if __name__ == "__main__":
    ...
    base_model_name = "Qwen/Qwen2.5-1B"
