...
def get_data(max_seq_src):
    print("\nFetching Dataset...")
    dataset = load_dataset("andythetechnerd03/AI-human-text", split='test').shuffle(seed=212)
    ...
    for data in dataset:
        _, tokenized_text = tokenize_with_tiktoken(unidecode(data["text"]).lower().strip())

        if len(tokenized_text) > max_seq_src:
            continue
        
        label_vector = [0] * num_labels
        label_vector[data['generated']] = 1
