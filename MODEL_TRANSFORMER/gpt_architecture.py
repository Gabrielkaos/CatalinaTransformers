...
class GPT2Attention(nn.Module):
    def __init__(self, d_model, n_heads, dropout, is_causal=True,use_flash_attn=True):
        super().__init__()
...
# we make the decoder act like an encoder by passing a mask and making it non_causal
class GPTEncoderTransformer(nn.Module):
    def __init__(self, d_model, decoder, embed, pos,num_class):
        super().__init__()
        self.decoder = decoder
        self.embed = embed
        self.pos = pos
...
def gpt2_like_model(
    ...
    for _ in range(n_layers):
        self_attn = GPT2Attention(d_model, n_heads, dropout, is_causal=True,use_flash_attn=flash_attn)
...
def gpt_classifier(
    ...
    decoder_blocks = []
    for _ in range(n_layers):
        self_attn = GPT2Attention(d_model, n_heads, dropout, is_causal=is_causal,use_flash_attn=flash_attn)
        ff = FeedForwardNet(d_model, 4 * d_model, dropout,activation=mlp_activation,bias=True)
        decoder_blocks.append(GPTDecoderBlock(self_attn, ff, dropout, d_model))

    decoder = GPTDecoder(nn.ModuleList(decoder_blocks),d_model)

    model = GPTEncoderTransformer(d_model,decoder, embed,pos,num_class)
