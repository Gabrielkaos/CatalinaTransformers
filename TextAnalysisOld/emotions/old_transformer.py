...
-this code has segment variable in InputEmbedding in my new code it doesnt have
-old uses PositionalEncoding basic while new uses Rotary Embedding
-this code doesnt have RMSNorm just layernorm
-in layer norm params have size 1 in new it has d_model
-in layer norm bias cannot be turned off meanwhile in new it can be
-in FFN bias cannot be turned off
-in FFN theres a single choice for activation
-MHA doesnt use flash attention
-ResidualConn doesnt have RMSNorm
-
"""


class InputEmbedding(nn.Module):
    def __init__(self, d_model, vocab_size, segment=False):
        super().__init__()
    ...
    def forward(self, x, segments=None):
        x = self.embedding(x) * math.sqrt(self.d_model)  # multiply by sqrt of dmodel
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, seq_len, dropout, device=None):
        ...
        pos_enc = torch.zeros(seq_len, d_model).to(device)
        ...
        self.register_buffer('pos_enc_buffer', pos_enc)

    def forward(self, x):
        if x.shape[1] < self.seq_len:
            sliced_pos_enc = self.pos_enc_buffer[:, :x.shape[0], :]
        else:
            sliced_pos_enc = self.pos_enc_buffer[:, :x.shape[1], :]
...
class MultiHeadBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        #shape becomes (batch_size, seq_len, n_heads, d_k)
...
class EncoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadBlock, feed_forward_block: FeedForwardNet, dropout):
        super().__init__()

        self.attention_block = self_attention_block
        self.feed_forward_block = feed_forward_block

        self.residual_conn = nn.ModuleList([ResidualConn(dropout) for _ in range(2)])
...
class OnlyDecoderBlock(nn.Module):
    def __init__(self, self_attention: MultiHeadBlock, feed_forward_block, dropout):
        ...
        self.residual_conns = nn.ModuleList([ResidualConn(dropout) for _ in range(2)])
...
class ProjectionLayerRegression(nn.Module):
    def __init__(self, d_model, vocab_size):
        super().__init__()
...
class TransformerEncoderOnly(nn.Module):
    ...
    def encode(self, src, src_mask, segments=None):
        src = self.src_embed(src, segments)
        src = self.src_pos(src)
        return self.encoder(src, src_mask)
...
class TransformerDecoderOnly(nn.Module):
    def __init__(self, decoder: OnlyDecoder, 
                 trgt_embed: InputEmbedding, 
    ...
    def decode(self, trgt, trgt_mask):
        trgt = self.trgt_embed(trgt)
        trgt = self.trgt_pos(trgt)
        return self.decoder(trgt, trgt_mask)

    def project(self, x):
        return self.proj(x)


def build_transformer_decoder(trgt_vocab_size, trgt_seq_length, 
                              d_model=512, n_layers=6, n_heads=8, 
                              dropout=0.1, dff=2048, device=None):
    ...
    for _ in range(n_layers):
        ...
        decoder_block = OnlyDecoderBlock(decoder_self_attention_block, feed_f_block, dropout)
        decoder_blocks.append(decoder_block)

    # Decoder
    decoder = OnlyDecoder(nn.ModuleList(decoder_blocks))

    # Projection layer
    projection_layer = ProjectionLayer(d_model, trgt_vocab_size)

    # Construct TransformerDecoderOnly
    transformer_decoder = TransformerDecoderOnly(decoder, trgt_embed, trgt_pos, projection_layer)
...
def build_transformer_regression(src_vocab_size, trgt_vocab_size, src_seq_length, d_model=512, n_layers=6,
    ...
    for _ in range(n_layers):
        ...
        feed_f_block = FeedForwardNet(d_model, dff, dropout)
        encoder_block = EncoderBlock(encoder_self_attention_block, feed_f_block, dropout)
    ...
    # project layer
    projection_layer = ProjectionLayerRegression(d_model, trgt_vocab_size)
...
#text to multi label classification
def build_transformer_encoder(src_vocab_size, trgt_vocab_size, src_seq_length, d_model=512, n_layers=6,
                          n_heads=8, dropout=0.1, dff=2048, device=None, is_segmented=False):
    # create embed layers
    src_embed = InputEmbedding(d_model, src_vocab_size, segment=is_segmented)
...
def build_transformer(src_vocab_size, trgt_vocab_size, src_seq_length, trgt_seq_length, d_model=512, n_layers=6,
                      n_heads=8, dropout=0.1, dff=2048, device=None, encoder_n_layer=None):
    
    if encoder_n_layer is None:
        encoder_n_layer = n_layers
...
class RNN(nn.Module):
    def __init__(self, n_input, n_hiddens, num_layers, n_output, device=None):
        ...
        self.lstm = nn.LSTM(n_hiddens, n_hiddens, num_layers, batch_first=True, bidirectional=True).to(device)
