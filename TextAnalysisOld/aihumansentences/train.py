    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 250 == 0:
                print(
                    f"[CHECKPOINT] Epoch:{epoch + 1}/{num_epochs}, Batch:{batch_idx + 1}/{len(train_loader)}, Loss:{loss.item():.4f}"
                )
                save_file(f"checkpoint/CHECKPOINT-{epoch + 1}-{batch_idx + 1}.pth", model)
        ...
        save_file(f"epoch/{epoch + 1}-{avg_loss:.4f}.pth", model)
